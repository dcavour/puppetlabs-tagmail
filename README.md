...
If the `lockfilepath` key is specified the lock files will be created in the directory specified.  If multiple masters are in use, this should point to a network share mounted on each of the masters.  If a single master is in use, the line is optional.
