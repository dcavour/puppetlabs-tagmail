...
If the `lockfilepath` key is specified the lock files will be created in the directory specified.  If the key is missing, the nodeconfig rules will be ignored.  If multiple masters are in use, this should point to a network share mounted on each of the masters.

The node rules can either be in the format of `node:exclude` or `node:interval:frequency`.  Interval is an integer; frequency can be one of the following:  s|second(s), m|minute(s), h|hour(s), d|day(s).  The exclude option will suppress all reports for the indicated node.  The interval and frequency options are multiplied together to limit how often a node sends reports.  The node will accept the `*` wildcard character.  Spaces are optional.  If a node matches multiple rules, the last matching rule will apply.
